...
let loadArticles () =
    asyncSeq {
        ...
        let res = document.DocumentNode.Descendants("article")
        for node in res do
            let href = node.Descendants("a") |> Seq.map(fun x -> x.GetAttributeValue("href", null)) |> Seq.head
            let detailsNode = node.ChildNodes |> Seq.filter(fun x -> x.HasClass("offer-item-details")) |> Seq.head
            let details = detailsNode.InnerText |> split('\n') |> Array.map(fun x -> x |> trim) |> Array.filter(fun x -> x |> String.IsNullOrEmpty |> not)
            yield {| details = details; url = href |}
    }
