...
module CsvStorage =
    let FirstRow =
        "Opis,Dzielnica,Ilość pokoi,Cena,Powierzchnia,Cena za m2,Data,Link do ogłoszenia"
    ...
    let private createCsv (elements: Offer seq) =
        ...
        elements
        |> Seq.map (row)
        |> Seq.fold (fun builder x -> builder |> StringBuilder.appendLine x) b
    ...
    let store (elements: Offer seq) =
        async {
            ...
            let csv =
                elements
                |> Seq.distinctBy (fun x -> x.Href)
                |> createCsv

            do! File.WriteAllTextAsync(fileName, csv)
                |> Async.AwaitTask
